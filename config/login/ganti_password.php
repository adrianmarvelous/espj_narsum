...
if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password_baru) < 8) {
  echo "<script>alert('Password harus mengandung 8 Karakter dan harus mengandung huruf kapital, angka, dan spesial karakter.');window.location='../../login_system/ganti_password.php?username=$username'</script>";
}
else{
  //echo 'Strong password.';
  $check_query = $db->prepare("SELECT nip, password, ganti_password FROM user_master_asn WHERE nip = :username");
  ...
  //if($check['password'] == $password){
    if($password_baru == $password_konfirmasi){
      ...
      echo "<script>alert('Konfirmasi Password Salah');window.location='../../login_system/ganti_password.php?username=$username'</script>";
    }
  //}
