...
if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password_baru) < 8) {
  echo "<script>alert('Password harus mengandung 8 Karakter dan harus mengandung huruf kapital, angka, dan spesial karakter.');window.location='../../login_system/ganti_password_tenaga_kontrak.php?username=$username'</script>";
}
else{
  ...
  //if($check['password'] == $password){
    if($password_baru == $password_konfirmasi){
      ...
      echo "<script>window.location='../../login_system/login_tahun.php'</script>";
    }
    else{
      echo "<script>alert('Konfirmasi Password Salah');window.location='../../login_system/ganti_password_tenaga_kontrak.php?username=$username'</script>";
    }
  //}
