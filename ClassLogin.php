...
Class Login{
		...
		if(isset($_COOKIE['adv_token'])){
			...
			if($cek){
				...
				if($row['ip'] == $_SERVER['REMOTE_ADDR'] || $row['useragent'] == $_SERVER['HTTP_USER_AGENT']){
					...
					$query_tenaga_kontrak = $this->db->query("SELECT nik, password, nama, ganti_password, role, id_bidang FROM user_tenaga_kontrak WHERE nik = ".$this->db->quote($username));
					...
					if($query_asn->rowCount() != 0){
						return array(
							...
							$_SESSION['nik'] = $data_query_asn['nik'],
					...
					elseif($query_tenaga_kontrak->rowCount() != 0){
						return array(
							...
							$_SESSION['tahun'] = $row['periode_tahun'],
							$_SESSION['role'] = $data_query_tenaga_kontrak['role'],
							$_SESSION['id_bidang'] = $data_query_tenaga_kontrak['id_bidang'],
	...
	Public function salah_login_action($username,$periode_tahun){
		...
		$save->execute(array(
			$tgl, $periode_tahun, $username, $ip, $useragent
		));
		return false;
	}
