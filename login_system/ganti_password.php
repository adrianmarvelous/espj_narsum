...
if (htmlentities(isset($_GET['username']))) {
    $username = htmlentities($_GET['username']);
    $check_query = $db->prepare("SELECT * FROM user_master_asn WHERE nik = :username");
    $check_query->bindParam(':username',$username);
    $check_query->execute();
    $check = $check_query->fetch(PDO::FETCH_ASSOC); 
    if ($check['username'] != $username) {
        echo "<script>alert('Username Tidak Ditemukan');window.location='../login_system/login_tahun.php';</script>";
    }
}
?>
<!DOCTYPE html>
...
<body>
    <div class="content">
        <div class="inner">
            <form method="post" action="../config/login/ganti_password.php">
                <p class="title">Untuk Pertama Login Silahkan Ganti Password Anda</p>
